...
export async function POST(request: Request) {
  try {
    ...
    let generationSource: "openai" | "fallback" | "saved" = existing ? "saved" : "fallback";
    let checkedAt = existing?.checked_at || new Date().toISOString();
    if (!intelligence || mode === "refreshed") { const generated = await generateIntelligence(question, match); intelligence = generated.intelligence; generationSource = generated.generationSource; checkedAt = await saveIntelligence(match.id, match.topic, intelligence, match.sources); }
    await logQuery(question, mode);
    return NextResponse.json({ intelligenceId: match.id, topic: match.topic, ...intelligence, freshness: "fresh", mode, generationSource, checkedAt, changedSources, retrievalScore: match.score, evidence: match.sources.map((s) => ({ ...s, excerpt: s.content })) });
