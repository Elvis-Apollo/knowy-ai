...
export type GenerationResult = { intelligence: Intelligence; generationSource: "openai" | "fallback" };
...
export function deterministicIntelligence(match: TopicMatch): Intelligence {
  if (match.id.startsWith("github-")) {
    ...
    return { answer: `The most recently updated open issues in ${match.topic.replace("GitHub · ", "")} are ${match.sources.map((source) => source.title.split(" · ")[0]).join(", ")}. The evidence below comes directly from GitHub and is versioned by each issue’s latest update.`, findings, confidence: .99, status: "live" };
  }
...
function normalizeGeneratedIntelligence(intelligence: Intelligence, match: TopicMatch): Intelligence {
  if (!match.id.startsWith("github-")) return intelligence;
  ...
  return {
    ...intelligence,
    answer: cleanPlainText(intelligence.answer),
    findings: intelligence.findings.slice(0, match.sources.length),
    status: "live",
  };
}
...
export async function generateIntelligence(question: string, match: TopicMatch): Promise<GenerationResult> {
  ...
  if (!apiKey) return { intelligence: fallback, generationSource: "fallback" };
  try {
    const githubFormatting = match.id.startsWith("github-") ? "\n\nGitHub output rules:\n- Write the answer as a genuinely synthesized two-sentence overview of the issue themes and what appears most important.\n- Use plain text only: no Markdown, numbered lists, bullets, backticks, or headings.\n- Do not use a fixed template such as ‘GitHub returned 3 issues.’\n- Put each individual issue in a separate finding with its source ID." : "";
    const response = await fetch("https://api.openai.com/v1/responses", { method: "POST", headers: { authorization: `Bearer ${apiKey}`, "content-type": "application/json" }, body: JSON.stringify({ model: process.env.OPENAI_MODEL || "gpt-5.6", input: `Answer only from the supplied evidence for ${match.topic}. Question: ${question}\n\nFreshness rules:\n- Determine the current state, not merely the historical reason.\n- When status evidence conflicts, the newer source version supersedes older status statements.\n- A source explicitly marked RESOLVED means the subject is no longer blocked unless newer evidence says otherwise.\n- Explain what changed when older context described a blocker that is now resolved.${githubFormatting}\n\nEvidence (including freshness metadata):\n${match.sources.map((s) => `[${s.id}] Source: ${s.source}; Updated: ${s.updatedAt}; Version: ${s.version}; Content: ${s.content}`).join("\n")}`, text: { format: { type: "json_schema", name: "company_intelligence", strict: true, schema: { type: "object", additionalProperties: false, required: ["answer", "findings", "confidence", "status"], properties: { answer: { type: "string" }, findings: { type: "array", items: { type: "object", additionalProperties: false, required: ["statement", "sourceIds"], properties: { statement: { type: "string" }, sourceIds: { type: "array", items: { type: "string" } } } } }, confidence: { type: "number" }, status: { type: "string" } } } } } }) });
    if (!response.ok) return { intelligence: fallback, generationSource: "fallback" };
    const outputText = extractOutputText(await response.json());
    return outputText ? { intelligence: normalizeGeneratedIntelligence(JSON.parse(outputText) as Intelligence, match), generationSource: "openai" } : { intelligence: fallback, generationSource: "fallback" };
  } catch { return { intelligence: fallback, generationSource: "fallback" }; }
}
