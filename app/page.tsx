...
type Result = {
  ...
  checkedAt: string; evidence: Evidence[]; changedSources?: string[]; generationSource?: "openai" | "fallback" | "saved";
};
        ...
        <div className="content">
          ...
          {!result ? (
            <div className="starterGrid">
              <button disabled={loading} onClick={() => ask(demoQuestion, "acme")}><span>01</span><strong>Acme deployment</strong><p>Why is the Acme deployment delayed?</p></button>
              <button disabled={loading} onClick={() => ask("What is the status of the OAuth migration?", "oauth")}><span>02</span><strong>OAuth migration</strong><p>What is the status of the OAuth migration?</p></button>
              <button disabled={loading} onClick={() => ask("Was the Northstar enterprise discount approved?", "pricing")}><span>03</span><strong>Enterprise pricing</strong><p>Was the Northstar discount approved?</p></button>
              <button disabled={loading} onClick={() => ask("What caused the Atlas API incident?", "atlas")}><span>04</span><strong>Atlas incident</strong><p>What caused the Atlas API incident?</p></button>
            ...
            <section className="answerCard" aria-live="polite">
              ...
              <div className="metaRow"><span><small>CONFIDENCE</small><strong>{result.confidence ? `${Math.round(result.confidence * 100)}%` : "—"}</strong></span><span><small>FRESHNESS</small><strong className={result.freshness === "fresh" ? "freshDot" : ""}>● {result.freshness}</strong></span><span><small>GENERATED BY</small><strong className={result.generationSource === "openai" ? "openaiSource" : ""}>{result.generationSource === "openai" ? "✦ OpenAI" : result.generationSource === "saved" ? "↗ Saved intelligence" : "Deterministic fallback"}</strong></span><span><small>LAST CHECKED</small><strong>Just now</strong></span></div>
