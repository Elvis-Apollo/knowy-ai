...
test("contains the finished Knowy product shell", async () => {
  ...
  assert.match(page, /Ask live GitHub/);
  assert.match(page, /Connecting to GitHub/);
  assert.match(page, /Retrieving and ranking current issues/);
  assert.match(page, /connectorProgress/);
...
test("contains the reusable intelligence and freshness pipeline", async () => {
  const [query, store, generator, page] = await Promise.all([
  ...
  assert.match(query, /searchGitHub/);
  assert.match(query, /generationSource/);
  assert.match(generator, /extractOutputText/);
  assert.match(generator, /newer source version supersedes older status statements/);
  assert.match(generator, /explicitly marked RESOLVED/);
  assert.match(generator, /normalizeGeneratedIntelligence/);
  assert.match(generator, /genuinely synthesized two-sentence overview/);
  assert.match(generator, /cleanPlainText/);
  assert.match(page, /GENERATED BY/);
  assert.match(page, /GitHub issue resolved/);
});
